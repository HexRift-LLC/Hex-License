...
function displayWelcome() {
  ...
  console.log(chalk.cyan("[System]"), `Environment: ${config?.server?.NODE_ENV || 'Production'}`);
 ...
 * @returns {Promise<boolean>} True if up to date
 */
...
async function connectDatabase() {
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      logger.error(`MongoDB connection error (attempt ${attempt}/${MAX_RETRIES}): ${err.message}`);
  ...
  try {
    ...
    for (const user of users) {
      try {
        ...
        logger.error(`Error updating user ${user.discordId}: ${error.message}`);
        updatedUsers.push({
          username: user.username,
          hasUpdate: false,
          error: error.message
    ...
    logger.info(`User synchronization complete. Updated users: ${updatedUsers.filter(u => u.hasUpdate).length}`);
  } catch (error) {
...
function registerProcessHandlers() {
  ...
  process.on('unhandledRejection', (reason, promise) => {
    ...
    // Log to Discord if available
    sendLog('server_error', {
      error: `Unhandled Promise Rejection: ${reason instanceof Error ? reason.message : reason}`,
...
async function startApplication() {
  try {
    ...
    console.error(chalk.red("[Error]"), `Startup failed: ${error.message}`);
    
    // Try to log to Discord if possible
    sendLog('server_error', {
      error: `Startup failure: ${error.message}`,
      stack: error.stack
    });
